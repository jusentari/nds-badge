...
# main targets
#---------------------------------------------------------------------------------
TOPDIR := /auto_mnt/shared/current_projects/jusentari_badge
...
#---------------------------------------------------------------------------------
	grit $< -fts -o$*
	identify -format '#define '$*'Width %w\n' $< >> $*.h
	identify -format '#define '$*'Height %h' $< >> $*.h
...
debug: $(BUILD)
	$(DESMUME) --disable-sound --arm9gdb 20000 nds-badge.nds

clean:
	rm -rf build nds-badge.elf nds-badge.nds
